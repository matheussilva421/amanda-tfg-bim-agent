...
**Autonomy model:** Maximum autonomy; human intervention only for UAC, authentication/licensing, irreducible missing source data, architectural approval gates, or external irreversible actions  
...
- silently choosing final architecture without a review gate;
...
Only one active BIM executor may modify a given RVT at a time.
...
The final implementation may replace a candidate only after tool evaluation.
...
Automatic fallback is permitted only to a provider already marked `PASS` or otherwise explicitly approved in the registry.
...
- require architectural selection before detailed BIM.
...
## 6.15 Human selection gate

Only a solution with:

```yaml
status: APPROVED_FOR_BIM
```

may enter detailed BIM compilation.
...
No `RELEASE_CANDIDATE` without persistence verification.
...
1. finish or explicitly suspend task;
2. run tests;
3. persist logs;
4. update project state;
5. update blockers;
6. save checkpoint where needed;
7. commit source/config changes;
8. write next task.
...
Only one writer may own the Revit production session.
...
Block modifications when the file path indicates source/master/GOLDEN.
...
- primary and fallback Revit capabilities are tested;
...
- BIM stages pass QA;
- accessibility and program checks pass to the supported extent;
...
# 12. Final approved architecture
...
This written specification has been checked for:
