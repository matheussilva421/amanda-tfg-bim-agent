...
## 4. Decisões que esta revisão não tomou pela Amanda

| Decisão | Evidência disponível | Efeito enquanto pendente |
|---|---|---|
| Programa/capacidade | PDF: 20 pessoas; XLSX Premissas!C15:C16 e C27: hipótese de 42 | Bloqueia baseline de produção; permite cenários separados |
| Tipologia/sigilo | Caderno e apoio discutem casa-abrigo/centro e visibilidade | Bloqueia adoção final da política de acessos/uso |
| Lote/norte/confrontações | Área textual, ilustrações e hipóteses | Implantação real depende de base geométrica suficiente |
...
| Arquitetura finalista | Sem seleção vinculada ao conteúdo verificada | Bloqueia R05–R16 detalhados de produção |
| Calendário acadêmico | Plano de apoio com semanas relativas | Confirmar regulamento/coordenação; não inventar datas |

Os apoios foram preservados. As contradições foram registradas sem reescrever a pesquisa ou escolher premissas em nome da autora. Fonte documentada, adoção projetual e comprovação normativa são decisões distintas.
...
[validate_documents.py](docs/review/validate_documents.py) executa 19 verificações estáticas: links/âncoras, entrada, caminho da especificação, status, conflito de programa, hipóteses, aprovação, suspensão, stubs, journal, capabilities, solver, backups, SDK, sintaxe de exemplos Python, cercas Markdown, grafo e IDs.

- Antes: 19 verificações; 3 passaram; 16 falharam por lacunas documentais.
- Depois: **19 verificações; 19 passaram; 0 falharam**. validation-after.json identifica os arquivos finais por hash.
...
Começar por [START_HERE_FOR_CODEX.md](START_HERE_FOR_CODEX.md) e pelo [handoff](docs/notes/2026-09-15-revisao-planos-handoff.md). Próxima implementação: M1/01, se solicitada. Resolver PROGRAM_BASELINE e demais decisões antes das tarefas dependentes; isso não bloqueia a revisão documental concluída nem o desenvolvimento sintético independente.
