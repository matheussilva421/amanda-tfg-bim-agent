...
### Task 1: Canonical migration preflight [P08-CAN-T01]

- [x] Read `START_HERE_FOR_CODEX.md`, canonical override, new migration spec and Plan 11.
- [x] `git fetch --all --prune`; inspect branches/worktrees/dirty files.
- [x] Run doctor/status and provider health.
- [x] Confirm current live state; if R12 legacy advanced beyond the packaged snapshot, record it but still apply supersedence.
- [x] Verify SHA-256 of program PDF and three canonical boards against `docs/source/SOURCE_MANIFEST.json`.
- [x] `NO_GO` if canonical sources are missing/corrupt.

### Task 2: Archive legacy linear R12 [P08-CAN-T02]

- [x] Close/save legacy RVT if open; obtain stable closed-file hash.
- [x] Preserve exactly one canonical historical copy as `archive/superseded-linear/AMANDA_LINEAR_R12_SUPERSEDED_REFERENCE.rvt` or equivalent project path.
- [x] Record stage, hash, source path, last known QA and reason `SUPERSEDED_BY_USER_DIRECTION`.
- [x] Do not delete linked evidence/journals needed to prove history.
- [x] Do not use archived geometry as the new model base.

### Task 3: Apply Plan 11 code/state migration [P08-CAN-T03]

- [x] Execute every task in `docs/superpowers/plans/11-canonical-pavilion-migration.md` using TDD.
- [x] Require migration-specific tests PASS; record established non-Revit baseline failures and confirm no new failures.
- [x] Require a new run/solution ID and content-bound approval hash that includes canonical reference hashes.

### Task 4: Reconcile site input without changing the canonical parti [P08-CAN-T04]

- [x] Ingest verified boundary/topography/north if available.
- [x] Otherwise continue as STUDY with `PROVISIONAL_ASSUMPTION` and existing blockers.
- [x] Map canonical "public edge" to verified/provisional Miguel Castro interface without copying street names/area from generated boards.
- [x] Never use missing site evidence as justification to return to a linear bar.

### Task 5: Generate canonical pavilion run [P08-CAN-T05]

- [x] Generate alternatives **only within** the fixed pavilion parti.
- [x] Required common invariants: admin public edge, 4 residential functional pavilions around central garden, child-green interface, separate service/capacitation block/access, covered external connections.
- [x] Variants may tune curvature, spacing, rotation and distribution, but cannot become a single bar.
- [x] Run hard program/geometry checks; preserve program areas exactly.
- [x] Save candidate geometry, metrics, canonical-conformance report and previews.

### Task 6: Select implementation inside the canonical parti [P08-CAN-T06]

- [x] Rank only canonical-conforming pavilion variants.
- [x] `selection_authority=USER_DIRECTED` for the parti itself.
- [x] `selection_authority=AGENT_DELEGATED` may select the best detailed implementation inside that parti.
- [x] Build new `approval_hash` from geometry + requirements + site profile + constraints + canonical board SHA-256 hashes.
- [x] `AMANDA_REVIEW_PENDING` remains nonblocking.
- [x] Record old selection as superseded; never edit its historical record in place.

### Task 7: Create clean canonical production RVT [P08-CAN-T07]
...
### Task 8: R01–R04 canonical massing [P08-CAN-T08]
...
### Task 10: R06 internal layout [P08-CAN-T10]
...
### Task 11: R07 openings and circulation [P08-CAN-T11]
...
### Task 12: R08 rooms/program reconciliation [P08-CAN-T12]
...
### Task 13: R09–R12 developed architecture [P08-CAN-T13]
...
### Task 14: R13 documentation [P08-CAN-T14]
...
### Task 15: R14 QA including canonical QA [P08-CAN-T15]
...
### Task 16: R15 Release Candidate + cold reopen [P08-CAN-T16]
...
### Task 17: Export and validate model-derived deliverables [P08-CAN-T17]
...
### Task 18: Promote R16 GOLDEN [P08-CAN-T18]
...
### Task 19: Cleanup and final handoff [P08-CAN-T19]
