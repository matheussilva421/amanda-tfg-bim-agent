...
Migrar o Amanda TFG BIM Agent da solução linear R12 para um novo run BIM cuja implantação, setorização e volumetria correspondam materialmente às quatro pranchas canônicas, preservando toda a infraestrutura/testes/providers já válidos.

## Source hierarchy

1. instrução atual do usuário;
2. `CANONICAL_PARTIDO_OVERRIDE_2026-09-22.md` + quatro pranchas canônicas;
...
- O approval hash da nova seleção deve incluir hashes das quatro imagens canônicas + programa + geometria gerada.
...
## Addendum — fourth canonical board

The fourth user-provided canonical image is the services/capacitation floor plan. It is a binding source for block organization, courtyard relationship, covered circulation, and the separate public and service/loading entries. Room areas printed on that concept board are approximate; the official program PDF remains authoritative for area and capacity. All active selection and approval hashes must bind four canonical images. Earlier three-image S02 acceptance is stale and cannot release R05.
