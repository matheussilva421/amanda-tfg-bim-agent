...
- Task 7 implementation is prepared on `codex/canonical-pavilion-migration` from `3569483`; commit and push are the immediate next actions. After that checkpoint, continue Plan 11 Task 8: add regression tests and harden `scripts/run_amanda_production.py` so superseded R12 cannot become the canonical target. Do not run the production script before that migration.
