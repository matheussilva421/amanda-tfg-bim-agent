...
Task 10 leaves the canonical candidate unpromoted and does not change `PROJECT_STATE.yaml`. Continue with Plan 11 Task 11: append the R12 supersedence record, select S02 in persistent project state only with the above proof attached, reset production stage status to pre-R04, preserve provider/site blockers, and set the rewritten canonical Phase 08 task as next. Before any Revit geometry write, reconcile the active target and writer lease and pass BIM-00. `CANONICAL_GEOMETRIC_ACCEPTANCE`, the R04 visual regression, the downstream R06/R08/R12/R13/R15 regressions, and the missing verified site/accessibility evidence remain open; R16 remains blocked.
